...
if __name__ == '__main__':
    ...
    for video in videos:
        ...
        if not os.path.exists(f'{data_dir}/bbox_smooth'):
            ...
            plt.figure()
            plt.plot(sizes, '-x', c='g')
            plt.plot(smoothed_sizes, '-o', c='r')
            plt.legend(['Original', 'Smoothed'])
            plt.title('Bounding Box Over Time', fontsize=20)

            plt.figure()
            plt.plot(centers[:, 0], '-x', c='g')
            plt.plot(smoothed_x, '-o', c='r')
            plt.legend(['Original', 'Smoothed'])
            plt.title('X Center Location Over Time', fontsize=20)

            plt.figure()
            plt.plot(centers[:, 1], '-x', c='g')
            plt.plot(smoothed_y, '-o', c='r')
            plt.legend(['Original', 'Smoothed'])
            plt.title('Y Center Location Over Time', fontsize=20)
            ...
            for i, frame in enumerate(images):
                ...
                if os.path.isfile(os.path.join(new_data_dir, frame.replace(".png", ".npy"))):
                    ...
                    # cv2.imwrite(f'../data/figures/{video}/smoothed_bbox/{i}.png', img)
